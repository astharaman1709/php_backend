...
<div class="main">
    <div class="col-sm-12 pad0 headdingnav">
        <div class="col-sm-6 pad0"><div class="headding">Press Gallery</div></div>
            ...
            <?php
                ...
                if (isset($_POST["submit"])) 
                {
                    ...
                    foreach ($_FILES["file"]["name"] as $i => $value) {
                        $files = rand().$_FILES["file"]["name"][$i];
                        $sql="insert into gallery_press values('','$files')";
                        $con->query($sql);
                        move_uploaded_file($_FILES['file']['tmp_name'][$i], "gallery_img/".$files);
                    }
                    
                    echo "<script>alert('Record Saved.');</script>";
                    echo "<script>window.location.href='press-gallery';</script>";
                }
            ?>
        </div>
        <div class="clear"></div>
    ...
    <div class="col-sm-12 pad0">
        <?
            $sql="SELECT * FROM gallery_press order by(id) desc";
    ...
    {   
        $(document).ready(function(){
            ...
            $.ajax({
                type: 'POST',
                url : 'delete',  
                data : {id : delid, tname : 'gallery_press'},
